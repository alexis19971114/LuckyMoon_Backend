...
import { getFilteredTokens } from "./controllers/contractInfo.js";
...
export const update = async () => {
  ...
  let first = true, price;

  // let address = "0x6942029e46a056d4a11011241a5a1a008c92f081"
  // let blockNumber = 19797876
  // price = await getPriceFromBlockAndToken(blockNumber, address)
  // console.log(price)
  ...
  for (let i = 0; i < len; ++ i) {
    ...
    if(tokens[i].firstBlockBuyCount <= 0) continue;

    let newToken = await tokenStructure.findOne({ address: tokens[i].address })
    newToken.price = await getPriceFromBlockAndToken(tokens[i].firstSwapBlockNumber, tokens[i].address)
    newToken.nonceToPriceRatio = (parseFloat(newToken.price) + parseFloat(100)) / tokens[i].firstBlockBuyCount;
    newToken.save()
  }
  console.log("done")
  console.log("Last Token: ", tokens[len - 1].address, tokens[len - 1].firstSwapBlockNumber)
  //Last Token:  0x0c5cbd10f973c45fd38098b83b945b59642443c3 19650774
};
